...
class User(object):
    ...
    def __init__(self, rank, old_rating, handle='', official_new_rating=None):
        self.rank = float(rank)
        self.old_rating = int(old_rating)
        self.seed = 1.0
        self.handle = str(handle)
        self.official_new_rating = official_new_rating
...
class RatingCalculator(object):
    ...
    def calculate(self):
        ...
        for user in self.user_list:
            if user.official_new_rating:
                user.delta = user.official_new_rating - user.old_rating
            else:
                user.delta = int(
